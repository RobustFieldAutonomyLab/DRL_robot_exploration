...
class experience_buffer():
    ...
    def add(self, experience):
        if len(self.buffer) + 1 > self.buffer_size:
            aa = 1 + len(self.buffer)
            bb = aa - self.buffer_size
            self.buffer[0:bb] = []
...
def padd_eps(eps_buff):
    if len(eps_buff) < trace_length:
        ...
        d = True
        h_state = np.zeros((1, h_size))
        c_state = np.zeros((1, h_size))
        for i in range(0, trace_length - len(eps_buff)):
            eps_buff.append(np.reshape(np.array([s, a, r, s1, d, h_state, c_state]), [1, 7]))
    return eps_buff
...
def start_training():
    ...
    while step_t <= EXPLORE:
        ...
        h_state1 = np.reshape(state1[0].cpu().detach().numpy(), (1, h_size))
        c_state1 = np.reshape(state1[1].cpu().detach().numpy(), (1, h_size))
        ...
        episodeBuffer.append(np.reshape(np.array([s_t, a_t, r_t, s_t1, terminal, h_state1, c_state1]), [1, 7]))
        ...
        if step_t > OBSERVE:
            ...
            h_state_batch = np.vstack(trainBatch[0:trace_length*BATCH:trace_length, 5]).reshape(1, BATCH, h_size)
            c_state_batch = np.vstack(trainBatch[0:trace_length*BATCH:trace_length, 6]).reshape(1, BATCH, h_size)
            state_batch = (torch.tensor(h_state_batch).to(device), torch.tensor(c_state_batch).to(device))

            readout_j1_batch, _ = test(s_j1_batch, 0., state_batch, target_net, trace_length, BATCH, h_size)
